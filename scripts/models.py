...
class LSTM_AE_fake(nn.Module):
  def __init__(self, num_ifos, num_timesteps, BOTTLENECK, FACTOR):
    super(LSTM_AE, self).__init__()
    self.num_timesteps = num_timesteps
    self.num_ifos = num_ifos
    self.BOTTLENECK = BOTTLENECK
    self.encoder = Encoder_fake(seq_len=num_timesteps, n_features=num_ifos, embedding_dim=BOTTLENECK)
    self.decoder = Decoder_fake(seq_len=num_timesteps, n_features=num_ifos, input_dim=BOTTLENECK)
  def forward(self, x):
